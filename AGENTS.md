...
- **Canonical id = `idstr`.** 2010-era posts have `id ≠ mid` (idstr `<老帖idstr已脱敏>` vs mid `<老帖mid已脱敏>`); `modifyVisible`/`destroy` take the idstr — use the `statusId()` helper, never `blog.mid` for operations (modern posts: id == idstr == mid).
