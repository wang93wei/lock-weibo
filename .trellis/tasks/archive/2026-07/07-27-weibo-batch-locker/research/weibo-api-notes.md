...
```jsonc
{
  "ok": 1,
  "data": {
    "since_id": <16位游标mid已脱敏>,   // 分页游标，下一页带上（示例值）
...
**单条微博关键字段**（实测首条 `id=<16位mid已脱敏>`）
```jsonc
{
  "id": <16位mid已脱敏>,            // 数字
  "idstr": "<16位mid已脱敏>",       // 字符串，与 id 同值
  "mid": "<16位mid已脱敏>",         // 字符串，与 id 同值
...
三个参数全部被服务端忽略**——实测对同一宽窗口（uid=<已脱敏>，2025 H1，total=134）
...
- **⚠️ searchProfile 索引覆盖不全（2026-08-29 实测，本账号 uid=<已脱敏>）**：
...
- **游标含边界推进（`nextEnd = oldestEpoch`）+ `seenMids` 去重**（2026-08-29 实测修订，
  uid=<已脱敏>）：旧的「`oldestEpoch - 1`（不含边界）」结论作废。-1 虽省一次边界条目
  ...
  正确性优先。
- **禁止用 `data.total` 做终止条件**（2026-08-29 实测推翻，uid=<已脱敏>；2026-07-28 的
...
（mid=<16位mid已脱敏> 实测命中）。对"批量锁定"场景完全可用——既不会漏掉已私有的（
...
- **PERM 微博可删已一手验证（2026-08-29）**：实测删除 idstr=<老帖idstr已脱敏>（2010-07-25 转发，
  `visible.type=0`，modifyVisible 报「暂不支持变更可见范围」）——首次调用 HTTP 200
  返回被删微博完整对象；二次调用返回 `{"ok":0,"message":"该微博不存在","error_code":20101}`。
- **⚠️ 老帖 `id` ≠ `mid`**：2010 年代老帖 idstr=<老帖idstr已脱敏> 而 mid=<老帖mid已脱敏>（18 位）。
...
## 9. 快转条目与 modifyVisible 失败契约（2026-08-12 实测）
...
## 10. 取消快转接口 destroy（2026-08-12 官方 bundle 核验）
